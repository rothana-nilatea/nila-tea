...
const pool = new Pool({
  ...
  // Connection pool settings — helps with Render cold starts
  max: 10,
...
// Retry helper — tries up to `maxRetries` times with exponential backoff
async function withRetry(fn, maxRetries = 5, label = 'DB operation') {
  let lastErr;
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      lastErr = err;
      const wait = Math.min(1000 * Math.pow(2, attempt - 1), 15000); // 1s, 2s, 4s, 8s, 15s
      console.error(`⚠️  ${label} failed (attempt ${attempt}/${maxRetries}): ${err.message}`);
